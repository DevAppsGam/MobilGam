...
//echo json_encode(['debug' => 'Prueba de respuesta desde PHP']);


// OBTENGO LA VARIABLE $conna
...
function obtenerFechaPromesa($conn, $folio)
{
    $sql = "SELECT fechaest FROM promesa WHERE folio=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $folio);
    $stmt->execute();
    $stmt->bind_result($fechaPromesa);  // Utilizar bind_result en lugar de get_result

    if ($stmt->fetch()) {
        $fechaPromesa = date('Y-m-d', strtotime($fechaPromesa));
    } else {
        $fechaPromesa = null;
    }
...
function calcularFechaVencimiento($fecha, $dias, $feriadosAlta)
{
    ...
    while ($i < $dias) {
        ...
        $esFeriado = isset($feriadosAlta[date('Y-m-d', $fechaVencimiento)]);
                    ...
                    if ($fechaPromesa) {
                        ...
                        while ($verfecha = mysqli_fetch_row($resultado)) {
                            ...
                            if ($row['estado'] == "TERMINADO CON POLIZA" || $row['estado'] == "TERMINADO") { //corrección en condiciones
                                ...
                                // Aquí irían las condiciones para ALTA DE POLIZA
                                switch ($filterItem) {
                                    case 'A TIEMPO':
